...
package com.slfinance.shanlincaifu.validate;
...
public class RulesCache {
	...
	public static Rules getAnnotationRulesByMethod(Method method) {
		Rules rules = null;
		if (method != null) {
			rules = annotationRules.get(method.getDeclaringClass().getName() + "." + method.getName());
			if (rules == null) {
				rules = method.getAnnotation(Rules.class);
				annotationRules.put(method.getDeclaringClass().getName() + "." + method.getName(), rules);
			}
		}
		return rules;
	}
